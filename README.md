...
**PrestoWise** bridges this gap by allowing riders to compare multiple Google Maps transit itineraries, calculate their total cross-agency trip costs, and see the amount saved through Ontario's One Fare Program.
...
* **Age Group Selection:** Calculates pricing for Adults, Seniors (>65), Youth (13–19), and Children (<12).
* **Polished UI/UX:** Features custom skeleton loaders for API latency, dynamic route pagination guarding against edge-case errors, and automated scrolling to view results. 

---

## 🛡️ Architecture

* **Frontend:** The Google Maps Javascript and Places API powers the autocomplete search widget. While frontend keys are inherently public, security is enforced via **HTTP Referrer domain restrictions** and disabling all other APIs.
* **Backend:** Routing calculations are offloaded to a **Vercel Serverless Function**, ensuring the Google Routes API key remains secure via encrypted environment variables.
...
1. **Plan a Trip:** Enter your origin and destination directly into the web app.
